...
def test_configured_entry_thresholds_preserve_other_limits():
    ...
    assert cfg.risk.per_trade_size_usdc == pytest.approx(1.5)
...
def test_take_profit_uses_entry_price_and_cost_buffer():
    ...
    assert "net=20.00%" in profitable.reason
    assert "target=20.00%" in profitable.reason
